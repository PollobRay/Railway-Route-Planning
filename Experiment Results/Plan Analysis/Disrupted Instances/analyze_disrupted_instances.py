#!/usr/bin/env python3
"""
Railway Planner Log Analyzer for Disrupted Instances (p101-p200)

Analyzes planner log files for disrupted railway planning instances.
Provides summaries per 10 instances (e.g., p101-p110, p111-p120, etc.)

Metrics Extracted:
- Quality Metrics: Makespan, Plan Length, Total Delay (including slowdown effects)
- Computation Metrics: Generation Time, States Evaluated
- Disruption-specific delay breakdown

Author: Railway Planning Research
Version: 2.0 - Disrupted Instances Focus
"""
...
@dataclass
class ProblemInfo:
    """Stores problem instance information for delay calculation."""
    num_trains: int = 0
    ...
    blocked_trains: List[str] = field(default_factory=list)
    blocked_tracks: List[Tuple[str, str]] = field(default_factory=list)
    damaged_engines: List[str] = field(default_factory=list)
    blockage_times: Dict[str, float] = field(default_factory=dict)
    track_clear_times: Dict[Tuple[str, str], float] = field(default_factory=dict)


class PlannerLogAnalyzer:
    """Analyzes OPTIC and POPF planner log files for disrupted instances."""
    ...
    def parse_problem_file(self, problem_path: str) -> ProblemInfo:
        """Parse PDDL problem file to extract distances, slowdowns, and disruptions."""
        info = ProblemInfo()
        ...
        # Extract blocked trains
        blocked_train_pattern = r'\(train-blocked (t\d+) (p\d+)\)'
        for match in re.finditer(blocked_train_pattern, content):
            train = match.group(1)
            info.blocked_trains.append(train)
        
        # Extract train blockage times
        blockage_time_pattern = r'\(= \(train-blockage-time (t\d+) (p\d+)\) (\d+(?:\.\d+)?)\)'
        for match in re.finditer(blockage_time_pattern, content):
            train, point, time = match.groups()
            info.blockage_times[train] = float(time)
        
        # Extract blocked tracks
        blocked_track_pattern = r'\(track-blocked (p\d+) (p\d+)\)'
        for match in re.finditer(blocked_track_pattern, content):
            p1, p2 = match.groups()
            if (p1, p2) not in info.blocked_tracks and (p2, p1) not in info.blocked_tracks:
                info.blocked_tracks.append((p1, p2))
        
        # Extract track clear times
        clear_time_pattern = r'\(= \(track-clear-time (p\d+) (p\d+)\) (\d+(?:\.\d+)?)\)'
        for match in re.finditer(clear_time_pattern, content):
            p1, p2, time = match.groups()
            info.track_clear_times[(p1, p2)] = float(time)
        
        # Extract damaged engines (trains with engine failure)
        damaged_pattern = r'\(engine-damaged (t\d+) (p\d+)\)'
        for match in re.finditer(damaged_pattern, content):
            train = match.group(1)
            info.damaged_engines.append(train)
        
        # Count trains
        train_pattern = r'\(train-at (t\d+)'
        info.num_trains = len(set(re.findall(train_pattern, content)))
    ...
    def calculate_delays(self, result: PlannerResult, problem_info: ProblemInfo) -> None:
        """Calculate total delay including slowdown effects."""
        ...
        for action in result.actions:
            ...
            if action.action_name in ['drive-train', 'drive-assisted-train']:
                # Parameters: train from to gauge (for drive-train)
                # Parameters: engine train from to gauge (for drive-assisted-train)
                ...
                if distance > 0 and speed > 0:
                    ...
                    # Actual time with slowdown: distance / (speed * (1 - slowdown))
    ...
    def analyze_all(self) -> None:
        """Analyze all disrupted instance log files (p101-p200)."""
        # Process disrupted instances (p101-p200)
        for i in range(101, 201):
            instance = f'p{i}'
            
            # Parse problem file for delay calculation
        ...
        print(f"Analyzed {len(self.results)} disrupted instances")
    ...
    def generate_batch_summary_table(self) -> str:
        ...
        lines = []
        lines.append('=' * 120)
        lines.append('DISRUPTED INSTANCES SUMMARY (p101-p200) - GROUPED BY 10')
        lines.append('=' * 120)
        lines.append('')
    ...
    def generate_overall_summary(self) -> str:
        ...
        lines = []
        lines.append('=' * 80)
        lines.append('OVERALL SUMMARY - DISRUPTED INSTANCES (p101-p200)')
        lines.append('=' * 80)
        lines.append('')
        
        # Overall metrics
        metrics = self.get_batch_metrics(101, 200, 'POPF')
        
        lines.append(f"Total Instances: {metrics['total']}")
        lines.append(f"Solved: {metrics['solved']} ({100*metrics['solved']/metrics['total']:.1f}%)")
        ...
        lines.append("Delay Breakdown:")
        lines.append(f"  Total Delay: {metrics['total_delay_sum']:.2f}")
        lines.append(f"    - Slowdown Delay: {metrics['slowdown_delay_sum']:.2f} "
                    f"({100*metrics['slowdown_delay_sum']/max(metrics['total_delay_sum'], 0.001):.1f}%)")
        lines.append(f"    - Blockage Delay: {metrics['blockage_delay_sum']:.2f} "
                    f"({100*metrics['blockage_delay_sum']/max(metrics['total_delay_sum'], 0.001):.1f}%)")
        lines.append(f"    - Engine Repair Delay: {metrics['engine_delay_sum']:.2f} "
                    f"({100*metrics['engine_delay_sum']/max(metrics['total_delay_sum'], 0.001):.1f}%)")
        lines.append('')
...
def main():
    parser = argparse.ArgumentParser(
        description='Analyze POPF planner logs for disrupted railway instances (p101-p200)'
    )
    parser.add_argument(
        '--directory', '-d',
        default='.',
    ...
    print('=' * 60)
    print('Railway Planner Log Analyzer - Disrupted Instances')
    print('Analyzing: p101 to p200')
    print('=' * 60)
